#!/bin/bash
#
# [QuickBox ARM64 Installation Guide Script]
#
...
function _askchsource() {
	if (whiptail --title "$INFO_TITLE_SOURCE" --yesno "$INFO_TEXT_SOURCE" --yes-button "$BUTTON_YES" --no-button "$BUTTON_NO" --defaultno 8 72); then
		chsource=1
		mirror=$(
			whiptail --title "$INFO_TITLE_SOURCE" --radiolist \
				"$INFO_TEXT_SOURCE_EXTRA" 12 36 6 \
				"default" "$CHOICE_TEXT_SOURCE_EXTRA_DEFAULT" on \
				"tuna" "$CHOICE_TEXT_SOURCE_EXTRA_TUNA" off \
				"ustc" "$CHOICE_TEXT_SOURCE_EXTRA_USTC" off \
				3>&1 1>&2 2>&3
		)
	else
		chsource=0
...
function _chsource() {
	...
	if [[ $DISTRO == Debian ]]; then
		if [[ $mirror == "tuna" ]]; then
			...
			cp ${local_setup_template}source.list/debian.ustc.template /etc/apt/sources.list
		else
		...
		if [[ $mirror == "tuna" ]]; then
			cp ${local_setup_template}source.list/raspbian.tuna.template /etc/apt/sources.list
		elif [[ $mirror == "ustc" ]]; then
			cp ${local_setup_template}source.list/raspbian.ustc.template /etc/apt/sources.list
		else
			cp ${local_setup_template}source.list/raspbian.template /etc/apt/sources.list
		fi
		sed -i "s/RELEASE/${CODENAME}/g" /etc/apt/sources.list
	else
		if [[ $mirror == "tuna" ]]; then
			...
			cp ${local_setup_template}source.list/ubuntu.ustc.template /etc/apt/sources.list
		else
